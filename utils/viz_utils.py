...
def visualize_attention_heatmap(
    ...
    Args:
        attention_matrix: 2D array/tensor of attention weights
        tokens: List of token strings corresponding to the sequence length
        title: Title for the heatmap plot
        save_path: Path to save the generated heatmap image
        colormap: Matplotlib colormap name
        max_tokens_display: Maximum number of token labels to display on each axis
    """
...
def visualize_processed_image_input(analysis_data: Dict[str, Any], save_dir: Optional[str] = None):
    ...
    Args:
        analysis_data: Dictionary containing analysis results with processed tensors
